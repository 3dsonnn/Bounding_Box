...
/*   Updated: 2025/02/04 04:23:53 by efinda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../../../inc/obx.h"

static void	grand_link_diagonals(t_obx *obx, int i, int j)
{
...
static void	grand_link_horizontal_boundary_tiles(t_obx *obx, int i, int j)
{
	...
	grand_link_diagonals(obx, 0, 0);
}

static void	grand_link_vertical_boundary_tiles(t_obx *obx, int i, int j)
{
	...
	grand_link_horizontal_boundary_tiles(obx, -1, -1);
}

static	void	grand_link_corners(t_obx *obx)
{
...
{
	grand_link_corners(obx);
	grand_link_vertical_boundary_tiles(obx, -1, -1);
	while (++i < obx->scene.map.size.y - 1)
	{
		j = 0;
		while (++j < obx->scene.map.size.x - 1)
